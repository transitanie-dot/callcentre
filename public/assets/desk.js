...
el('escGo').addEventListener('click', async function () {
  ...
  try {
    ...
    /**
     * E a fila de escaladas.
     *
     * O loadDesk carrega a fila normal — a conversa que acabou de
     * ser escalada sai de lá e não aparece em lado nenhum até
     * alguém recarregar a página.
     *
     * Para um supervisor que escala a si próprio, isso significa
     * ver a conversa desaparecer sem explicação.
     */
    await carregarEscaladas();
  } catch (e) {
...
function pintarDia() {
  ...
  if (desk.state && desk.state !== 'unknown' && desk.state !== 'offline') {
    /**
     * O tempo no estado atual vem do RELÓGIO, não do que o dia
     * trouxe.
     *
     * O tempoDoDia é carregado de dois em dois minutos. Entre
     * carregamentos, somar só o que passou desde a resposta dá um
     * número diferente do que o botão mostra — e os dois ficam a
     * discordar no mesmo ecrã.
     *
     * Pior: um estado em que se entrou agora não está no
     * tempoDoDia de todo, e a barra mostrava zero enquanto o botão
     * já contava. Era o caso do lunch.
     *
     * O segundosNoEstado() é a mesma fonte que o botão usa. Assim
     * os dois dizem o mesmo, sempre.
     */
    var noEstado = segundosNoEstado();

    /**
     * O que já lá estava HOJE, tirando o período atual.
     *
     * O agent_day_states inclui o período a decorrer. Somar o
     * relógio por cima contaria o mesmo tempo duas vezes.
     */
    var antes = Math.max(0, (tempoDoDia[desk.state] || 0) - noEstado);

    vivo[desk.state] = antes + noEstado;
  }
...
function pintarPerfil(d) {
  ...
  /**
   * A função chama-lhe "requirements", não "required".
   *
   * O painel lia o nome errado e recebia sempre uma lista vazia —
   * e uma lista vazia de documentos exigidos desenha-se como "sem
   * requisitos para este país", que parece uma resposta e não é.
   */
  var exigidos = d.requirements || d.required || [];
